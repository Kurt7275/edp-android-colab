    ...
    Column(Modifier.fillMaxSize().systemBarsPadding().padding(24.dp), verticalArrangement = Arrangement.spacedBy(16.dp)) {
        Card(colors = CardDefaults.cardColors(containerColor = Color(0xFFE8F5E9)), modifier = Modifier.fillMaxWidth()) {
            Column(Modifier.padding(16.dp)) {
                Text("You successfully logged in!", style = MaterialTheme.typography.titleMedium, color = Color(0xFF2E7D32))
                Text("Welcome back, ${user.fullName}.", color = Color(0xFF1B5E20))
            }
        }
        ...
        ageFrom(user.birthdate)?.let { ProfileRow("Age", it.toString()) }
...
fun ageFrom(birthdate: String): Int? = runCatching {
